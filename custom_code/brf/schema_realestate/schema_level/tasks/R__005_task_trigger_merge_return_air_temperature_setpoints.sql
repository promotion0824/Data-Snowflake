-----------------------------------------------------------------------------------------------------
-- Task that triggers return_air_temperature_setpoints data refresh
-- Executed after task transformed.trigger_merge_return_air_temperature_assets_tk
-- --------------------------------------------------------------------------------------------------
...
SET task_schedule = 'USING CRON 0 */3 * * * ' || COALESCE($time_zone,$time_zone_default);
...
CREATE OR REPLACE TASK transformed.trigger_merge_return_air_temperature_setpoints_tk
...
AS
  CALL transformed.merge_return_air_temperature_setpoints_sp(SYSTEM$CURRENT_USER_TASK_NAME()); 
