...
CREATE OR REPLACE PROCEDURE transformed.occupancy_merge_building_hourly_sp()
  ...
  AS
    ...
    SELECT DISTINCT
        ...
        LAST_VALUE(ts.telemetry_value ) OVER (PARTITION BY t.building_id, t.model_id, datetime_local_hour ORDER BY ts.timestamp_local) AS end_of_hour_value,
    ...
    WHERE ts.timestamp_local >= (SELECT DATEADD(HOUR,-3,IFNULL(MAX(max_datetime_local_hour),'2024-01-01')) FROM watermark)
    )
    SELECT
        ts.building_id,
        t.building_name,
