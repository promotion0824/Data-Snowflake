...
SELECT
	...
	v.site_id,
	v.site_name,
	LEAST(sysdate(),MAX(ts.start_of_hour) OVER (PARTITION BY ts.trend_id)) AS last_captured_at_local,
	--LEAST(sysdate(),MAX(ts._last_updated_at) OVER()) AS last_refreshed_at_utc (we can't use this since data is pre-populated with future dates)
    last_captured_at_local AS last_refreshed_at_utc ,
    last_captured_at_local AS last_refreshed_at_local
