...
SELECT
	...
	ts.unit,
	CASE WHEN ts.model_id NOT LIKE '%Cost%' THEN ts.energy_consumption_mmBTU * .29307107 ELSE NULL END AS energy_consumption_MWh,
	...
	CASE    WHEN ts.service_type = 'Electricity' AND ts.model_id NOT LIKE '%Cost%' THEN utility_billed_amount * ts.emissions_factor 
			WHEN ts.service_type = 'Natural Gas' AND ts.model_id NOT LIKE '%Cost%'  THEN energy_consumption_MWh  * ts.emissions_factor
			ELSE NULL 
