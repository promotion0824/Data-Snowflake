...
WITH cte_cbecs AS (
    SELECT score, all_buildings AS cbecs_value, LEAD(all_buildings,1) OVER (ORDER BY score desc) AS next_value
    FROM transformed.sustainability_cbecs_score 
    UNION ALL
    SELECT 100, 0 AS cbecs_value, MIN(all_buildings) AS next_value 
    FROM transformed.sustainability_cbecs_score 
    UNION ALL
    SELECT 0, MAX(all_buildings) AS cbecs_value, 999999 AS next_value 
    FROM transformed.sustainability_cbecs_score 
)
...
SELECT DISTINCT
    ...
    'kWh' AS energy_unit, 
    building_gross_area,
    CASE WHEN resource_type ILIKE '%Water%' THEN conformed_unit || '/' || REPLACE(building_gross_area_unit,'squareMeter','m2') ELSE 'kWh/'|| REPLACE(building_gross_area_unit,'squareMeter','m2') END AS normalization_unit,
    ...
    '$/'|| REPLACE(building_gross_area_unit,'squareMeter','m2') AS normalized_cost_unit,
...
SELECT DISTINCT
    ...
    building_gross_area,
    'kWh/'|| REPLACE(MAX(building_gross_area_unit),'squareMeter','m2') AS normalization_unit,
    ...
    '$/'|| REPLACE(MAX(building_gross_area_unit),'squareMeter','m2') AS normalized_cost_unit,
...
SELECT DISTINCT
    ...
    'kWh/m2-'|| deg.temperature_unit || '-day' AS normalization_unit,
