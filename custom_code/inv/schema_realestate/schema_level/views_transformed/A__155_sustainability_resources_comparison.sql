...
SELECT DISTINCT
	...
	end_period_value - COALESCE(adj_beginning,begin_period_value) AS resource_usage,
	t.unit,
	t.unit AS conformed_unit,
	resource_usage AS amount,
	...
	t.data_type,
	t.emissions_factor,
	NULL AS emissions_scope,
	NULL AS energy_consumption,
	NULL AS energy_unit,
	NULL AS carbon_emissions,
	NULL AS emissions_factor_unit,
	time_zone,
	...
	WHERE t.resource_type = 'Domestic Water' AND t.data_type = 'Actual' AND billing_period_start IS NOT NULL
)
,cte_monthly_total AS (
    SELECT 
		...
		END) AS amount,
		'MWh' AS energy_unit,
		CASE WHEN rt.resource_type = 'Electricity' THEN amount
			 WHEN rt.resource_type = 'Natural Gas' THEN amount * .29307107
			 WHEN rt.resource_type = 'Steam' THEN amount * 1.194 * .29307107
...
, cte_consumption AS (
	...
	SELECT
		...
		b.city,
		CONVERT_TIMEZONE( 'UTC', time_zone, MAX(ts.last_refreshed_at_utc) OVER ()) AS last_refreshed_at_local
	FROM cte_actual_domestic_water ts
	JOIN cte_building b ON ts.building_id = b .building_id	
)
SELECT DISTINCT
	c.month_start_date,
