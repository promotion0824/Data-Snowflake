...
-- custom code to eliminate duplicates
-- ******************************************************************************************************************************
...
CREATE OR REPLACE PROCEDURE transformed.sustainability_create_table_utility_bills_sp()
  ...
  AS
	...
	BEGIN
        ...
        WHERE --(trend_id IN (select trend_id FROM transformed.sustainability_twins) OR
              external_id IN (select external_id FROM transformed.sustainability_twins WHERE external_id > '')
              --)
