...
SELECT DISTINCT
        ...
        COALESCE(ca.gross_area, b.gross_area, b.rentable_area) AS building_gross_area,
        COALESCE(ca.gross_area_unit, b.gross_area_unit, 'sf') AS building_gross_area_unit,
   ...
   FROM transformed.sustainability_resources_models m
        JOIN transformed.capabilities_assets ca 
          ON m.capablity_model_id = ca.model_id AND ca.model_id_asset NOT IN ('dtmi:com:willowinc:Equipment;1')
   ...
   LEFT JOIN transformed.buildings b 
          ON (ca.building_id = b.building_id)
    WHERE 
    ...
    QUALIFY ROW_NUMBER() OVER (PARTITION BY ca.id,ca.asset_id ORDER BY ca._staged_at DESC) = 1
;
