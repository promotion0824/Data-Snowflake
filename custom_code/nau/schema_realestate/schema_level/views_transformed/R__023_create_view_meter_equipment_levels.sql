...
-- ******************************************************************************************************************************
CREATE OR REPLACE VIEW transformed.meter_equipment_levels AS
	SELECT DISTINCT
        a.asset_id,
		de.asset_class,
		...
		JOIN transformed.meter_equipment de 
		  ON (a.asset_id = de.asset_id)
		LEFT JOIN transformed.twins_relationships_deduped tr 
		  ON (a.asset_id = tr.source_twin_id)
		LEFT JOIN transformed.meter_equipment t 
		  ON (tr.target_twin_id = t.asset_id)
		LEFT JOIN transformed.twins_relationships_deduped tr2 
		  ON (t.asset_id = tr2.source_twin_id)
		LEFT JOIN transformed.meter_equipment t2
		  ON (tr2.target_twin_id = t2.asset_id)
		WHERE
			IFNULL(tr.relationship_name,'serves') = 'serves'
        -- AND IFNULL(tr2.relationship_name,'isFedBy') IN ('isFedBy','locatedIn')
        -- -- AND (level_1_model_id IN ('dtmi:com:willowinc:ElectricalPanelboard;1','dtmi:com:willowinc:Switchboard;1','dtmi:com:willowinc:Switchgear;1','dtmi:com:willowinc:ElectricalPanelboardMLO;1','dtmi:com:willowinc:ElectricalPanelboardMCB;1') OR level_1_model_id IS NULL)
        -- -- AND (level_2_model_id IN ('dtmi:com:willowinc:ElectricalPanelboard;1','dtmi:com:willowinc:Switchboard;1','dtmi:com:willowinc:Switchgear;1','dtmi:com:willowinc:ElectricalPanelboardMLO;1','dtmi:com:willowinc:ElectricalPanelboardMCB;1') OR level_2_model_id IS NULL)
