        ...
        CREATE OR REPLACE TEMPORARY TABLE new_run_sensor_values AS
          WITH cte_value_changes AS (
            ...
            SELECT
                ...
                model_id AS run_sensor_model_id,
            ...
            JOIN transformed.capabilities_assets assets 
                ON (new_run_sensor.site_id  = assets.site_id)
               AND (new_run_sensor.trend_id = assets.trend_id)
            WHERE new_run_sensor.timestamp_utc > IFNULL(:watermark, TO_TIMESTAMP('0000-01-01'))
              AND model_id in (
                    'dtmi:com:willowinc:CompressorRunSensor;1'
            ) 
