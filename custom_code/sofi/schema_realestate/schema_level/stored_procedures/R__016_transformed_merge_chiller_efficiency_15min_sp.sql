-- ******************************************************************************************************************************
-- Stored procedure that incrementally populates the chiller_efficiency_15mins table
-- ******************************************************************************************************************************
CREATE OR REPLACE PROCEDURE transformed.merge_chiller_efficiency_15min_sp(task_name VARCHAR)
    RETURNS STRING
    LANGUAGE SQL
  AS
    $$
      BEGIN
        LET watermark TIMESTAMP_NTZ := (SELECT MAX(date_time_local_15min) FROM transformed.chiller_efficiency_15min);
               
        MERGE INTO transformed.chiller_efficiency_15min AS tgt 
          USING (
            SELECT
              scores.asset_id,
              scores.site_id,
              ...
              scores.unit,
              scores.sensor_type,
              AVG(CASE WHEN chiller_run_status=TRUE THEN sensor_value ELSE NULL END) AS avg_sensor_value,
              COUNT_IF(chiller_run_status) AS count_run_sensor_on,
              COUNT_IF(chiller_run_status=FALSE) AS count_run_sensor_off
            FROM transformed.chiller_efficiency_measurements scores             
          ...
          WHEN MATCHED THEN
            ...
            SET
              ...
              tgt.avg_sensor_value = src.avg_sensor_value,
