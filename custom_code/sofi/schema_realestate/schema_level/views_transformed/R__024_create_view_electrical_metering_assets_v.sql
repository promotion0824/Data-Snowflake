...
SELECT DISTINCT
		...
		ca.asset_name,
		CASE WHEN  ca.id IN (
			'HP-SOFI-ST-L06-MSS-ESB1-103A-Main-ElecTotalActiveEnergySensor-Calc',
			'HP-SOFI-ST-L06-MSS-ESB2-209A-Main-ElecTotalActiveEnergySensor-Calc'
		)
		THEN 'YouTube Theater' ELSE ca.comments END AS asset_display_name,
		...
		CASE WHEN capability_model_class IN ('dtmi:com:willowinc:ActiveElectricalPowerSensor;1' ) THEN 'Power' 
			 WHEN capability_model_class IN ('dtmi:com:willowinc:ActiveElectricalEnergySensor;1') THEN 'Energy' 
			 ELSE null 
	    END AS sensor_type,
