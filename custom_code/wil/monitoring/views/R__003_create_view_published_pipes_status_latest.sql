-- ******************************************************************************************************************************
-- Create Pipes Status Latest view
-- ******************************************************************************************************************************
CREATE OR REPLACE VIEW central_monitoring_db.published.pipes_status_latest AS 
  SELECT 
  ...
  QUALIFY ROW_NUMBER() OVER (PARTITION BY pipes_status.account_name, name, schema_name, database_name ORDER BY pipes_status._captured_at DESC) = 1
;