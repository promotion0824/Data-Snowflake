...
-- Load toun files
-- ******************************************************************************************************************************
CREATE OR REPLACE PROCEDURE transformed.merge_tou_rggi_auction_sp()
    RETURNS STRING
    LANGUAGE SQL
	EXECUTE AS CALLER
  AS
    $$
		BEGIN
			...
			COPY INTO raw.stage_tou_rggi_auction
				FROM  (SELECT $2,$3,$4,$5,$6,$7,$8,metadata$filename,SYSDATE() FROM @raw.ADHOC_ESG/toun/auction_results/auction_results.csv)
