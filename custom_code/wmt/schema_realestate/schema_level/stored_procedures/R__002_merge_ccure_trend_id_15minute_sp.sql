...
CREATE OR REPLACE PROCEDURE transformed.merge_ccure_trend_id_15minute_sp(task_name VARCHAR)
    RETURNS STRING
    LANGUAGE SQL
  AS
    $$
	BEGIN
	  ...
	  MERGE INTO transformed.ccure_trend_id_15minute AS tgt
		USING ( 
			  WITH watermark AS 
				(
				  SELECT DISTINCT
					IFNULL(MAX(date_time_local_15min),'2019-01-01') AS max_15min
				  FROM transformed.ccure_trend_id_15minute
				)
			SELECT DISTINCT
			...
			JOIN (SELECT site_id,trend_id,min(date_local) AS min_date, max(date_local) AS max_date
				  FROM transformed.ccure_time_series_15minute GROUP BY site_id,trend_id
