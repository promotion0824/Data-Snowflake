...
CREATE OR REPLACE SECURE VIEW published.telemetry AS
WITH cte_lucernix AS (
	SELECT building_id, MAX(custom_properties:externalIds.Real_Estate_ID::STRING) as LucernixId
	FROM transformed.buildings building_id
...
BEGIN
	...
	IF (currentdb='PRD_DB') THEN
		GRANT SELECT ON VIEW published.telemetry TO SHARE external_share;
	END IF;
END;
$$