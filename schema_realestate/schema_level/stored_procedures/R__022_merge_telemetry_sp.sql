      ...
      BEGIN
		   ...
		   USING (
			SELECT
                ...
                TO_DATE(timestamp_local) AS date_local,
				...
				COALESCE(ts.connector_id,t.connector_id) AS connector_id,
                t.twin_id AS dt_id,
				ts.enqueued_at,
				ts.latitude,
				ts.longitude,
			...
			JOIN (SELECT DISTINCT trend_id,site_id,external_id,twin_id,connector_id
           ...
           UNION ALL
            ...
            QUALIFY ROW_NUMBER() OVER (PARTITION BY ts.external_id, ts.captured_at ORDER BY enqueued_at DESC) = 1         
			 ...
			 ON (tgt.timestamp_utc = src.timestamp_utc)
