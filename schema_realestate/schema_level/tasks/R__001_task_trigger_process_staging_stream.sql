-- ------------------------------------------------------------------------------------------------------------------------------
-- Process staging stream (root task) into raw tables
-- ------------------------------------------------------------------------------------------------------------------------------
...
AS
  INSERT ALL
    ...
    WHEN _schema_type = 'InsightDB/InsightOccurrences' THEN
      INTO raw.json_insight_occurrences VALUES (_stage_record_id, json_value, _stage_file_name, _loader_run_id, _ingested_at, _staged_at)
    ...
    WHEN _schema_type = 'Willow/DigitalTwin/Adx/Models' THEN
      INTO raw.json_ontology_models VALUES (_stage_record_id, json_value, _stage_file_name, _loader_run_id, _ingested_at, _staged_at)
