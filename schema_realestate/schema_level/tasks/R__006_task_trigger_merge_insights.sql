...
SET time_zone_default = (SELECT CASE current_region()
    ...
    ELSE NULL
    END);
SET task_schedule = 'USING CRON 10 */8 * * * ' || COALESCE($time_zone,$time_zone_default,'Etc/UTC');

CREATE OR REPLACE TASK raw.merge_insights_stream_tk
...
WHEN
  SYSTEM$STREAM_HAS_DATA('json_insights_str')
AS
  CALL transformed.merge_insights_sp(SYSTEM$CURRENT_USER_TASK_NAME())
;      
