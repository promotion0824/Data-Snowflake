...
SET time_zone_default = (SELECT CASE current_region()
    ...
    ELSE NULL
    END);
SET task_schedule = 'USING CRON 0 15 * * * ' || COALESCE($time_zone,$time_zone_default,'Etc/UTC');
CREATE OR REPLACE TASK raw.trigger_merge_ontology_models_tk
  USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
  --  At minute 15 past midnight UTC
  SCHEDULE = $task_schedule
  USER_TASK_TIMEOUT_MS = 1200000
  SUSPEND_TASK_AFTER_NUM_FAILURES = 5
  ERROR_INTEGRATION = error_{{ environment }}_nin
WHEN
  SYSTEM$STREAM_HAS_DATA('json_ontology_models_str')
AS
  CALL transformed.merge_ontology_models_sp(SYSTEM$CURRENT_USER_TASK_NAME())
;      
    
ALTER TASK raw.trigger_merge_ontology_models_tk RESUME;