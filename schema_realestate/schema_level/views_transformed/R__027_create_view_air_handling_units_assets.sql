...
-- ******************************************************************************************************************************
CREATE OR REPLACE VIEW transformed.air_handling_unit_assets AS

	WITH cte_air_handling_units_and_descendants AS (

		SELECT
		  DISTINCT model_id, extends_model_id,display_name_en
		FROM transformed.ontology_model_hierarchy
		START WITH model_id = 'dtmi:com:willowinc:AirHandlingUnit;1'
	...
	SELECT
		   ...
		   'dtmi:com:willowinc:AirHandlingUnit;1' AS asset_class,
		   display_name_en,
		   floor_id,
		   t.site_id
	FROM cte_air_handling_units_and_descendants ah
