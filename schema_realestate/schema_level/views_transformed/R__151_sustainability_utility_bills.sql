...
,cte_consumption AS (
    ...
    JOIN transformed.sustainability_twins a ON (ts.external_id = a.external_id OR ts.trend_id = a.trend_id)
)
...
JOIN transformed.sustainability_twins a ON (ts.external_id = a.external_id OR ts.trend_id = a.trend_id)
LEFT JOIN cte_consumption_monthly m1 ON DATEADD('month', -1,ts.month_start_date) = m1.month_start_date AND (ts.external_id = m1.external_id OR ts.trend_id = m1.trend_id)
LEFT JOIN cte_consumption_monthly y1 ON DATEADD('month',-12,ts.month_start_date) = y1.month_start_date AND (ts.external_id = y1.external_id OR ts.trend_id = y1.trend_id)
;
